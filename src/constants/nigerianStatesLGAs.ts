...
export const NIGERIAN_STATES_LGAS: StateData[] = [
  {
    state: 'Abia',
  ...
  {
    state: 'Anambra',
    lgas: ['Aguata', 'Awka North', 'Awka South', 'Ayamelum', 'Dunukofia', 'Ekwusigo', 'Idemili North', 'Idemili South', 'Ihiala', 'Njikoka', 'Nnewi North', 'Nnewi South', 'Ogbaru', 'Onitsha North', 'Onitsha South', 'Orumba North', 'Orumba South', 'Oyi'],
  },
  {
    state: 'Bauchi',
...
export const NIGERIAN_STATES = NIGERIAN_STATES_LGAS.map((s) => s.state);

export function getLGAsForState(state: string): string[] {
  return NIGERIAN_STATES_LGAS.find((s) => s.state === state)?.lgas ?? [];
}